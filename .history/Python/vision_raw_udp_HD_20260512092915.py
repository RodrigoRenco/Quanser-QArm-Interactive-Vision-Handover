...
# Opción 1: si renombraste la carpeta a Detection_Models
from Detection_Models.hands_detector import (
...
# Historial para suavizado del gesto
historial_gestos = []
HIST_SIZE = 5
# =========================================================================
...
# Mapeo de gestos a enteros (para enviar por UDP)
GESTO_TO_INT = {
    "NONE": 0,
    "STOP": 1,
    "OK": 2,
...
def obtener_gesto(hand_landmarks):
    ...
    if es_palm_stop(hand_landmarks):
        ...
        return "HAND"   # Hay mano pero no gesto especial
...
try:
    while True:
        ...
        if results_hands.multi_hand_landmarks:
            ...
            # Suavizado temporal
            historial_gestos.append(gesto_actual)
            if len(historial_gestos) > HIST_SIZE:
                historial_gestos.pop(0)
            # Gestor final = el más frecuente en el historial
            from collections import Counter
            gesto_str = Counter(historial_gestos).most_common(1)[0][0]
        else:
        ...
        # 4. UDP Communication (AHORA CON 8 FLOATS: + gesto)
        try:
